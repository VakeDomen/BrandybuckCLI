...
pub fn generate_folder(dir: String) -> () {
    ...
    match result {
        ...
        Err(e) => eprintln!(
            "Oops an error occured while generating folder {}: {}",
            dir, e
        ),
    }
}
...
pub fn copy_file(source: String, destination: String) -> () {
    ...
    match result {
        ...
        Err(e) => eprintln!(
            "Oops an error occured while trying to copy file from {} to {}: {}",
    ...
    match file {
        Ok(mut f) => {
            ...
            match result {
                ...
                Err(e) => eprintln!(
                    "Oops an error occured while trying to write to file file {}: {}",
                    destination, e
                ),
            }
        }
        Err(e) => eprintln!(
            "Oops an error occured while trying to create file {}: {}",
            destination, e
        ),
    }
}
...
pub fn read_file(filepath: String) -> String {
    let file = File::open(filepath.to_string()).expect("could not open file");
...
pub fn read_model_file(filepath: String) -> ModelFile {
    ...
    match config_file {
        ...
        Err(e) => {
            ...
            panic!(
                "Error loading a {}. If there is none, consider using the 'init' command",
                filepath
            );
        }
    }
